...
public class AddGallery implements Initializable {
  ...
  @FXML private Button add_gallery;

  @FXML private TextField description_id;

  @FXML private ChoiceBox<String> ending_hour;

  @FXML private ChoiceBox<String> location_id;

  @FXML private TextField phone_id;

  @FXML private Text price_alert;

  @FXML private ChoiceBox<String> starting_hour;
  ...
  void addImage(ActionEvent event) {
    // Load the image (replace "/path/to/your/image.jpg" with the actual file
    // path)
  ...
  private final String[] hours = {"00", "01", "02", "03", "04", "05", "06",
  ...
  void addGallery(ActionEvent event) {
    ...
    try {
      if (validateFields()) {
        ...
        GS.addd(new gallery(Name_id.getText(), description_id.getText(),
                            location, phone, operating_hours));
        ...
        try {
          root =
              FXMLLoader.load(getClass().getResource("/View_Galleries.fxml"));
        } catch (IOException e) {
