...
public class ShowEvents implements Initializable {
  ...
  public List<LocalDate> getDatesWithEvents() {
    ...
    try (Connection connection = MyDatabase.getInstance().getConn()) {
      ...
      PreparedStatement statement =
          connection.prepareStatement(SQL_SELECT_EVENT_DATES);
      // Execute query
      ...
      while (resultSet.next()) {
        ...
        LocalDate eventDate = LocalDate.parse(
            dateString, DateTimeFormatter.ofPattern(DATE_FORMAT_PATTERN));
  ...
  void show_calendar(ActionEvent event) {
      ...
      try {
          ...
          ObservableList<LocalDate> disabledDates =
  FXCollections.observableArrayList(datesWithEvents);
          ...
          // Close the database connection in a finally block to ensure it's
  always closed if (connection != null) { try { connection.close(); } catch
...
*/
  ...
  private void fetchWeatherData(String apiKey, String locationKey)
      throws Exception {
    String apiUrl = "http://dataservice.accuweather.com/currentconditions/v1/" +
    ...
    if (jsonArray.length() > 0) {
      ...
      double celsiusTemperature =
          temperature.getJSONObject("Metric").getDouble("Value");
  ...
  private void displayWeatherInfo(String weatherDescription,
                                  double temperature) {
