...
public class AddAuction implements Initializable {
  ...
  @FXML private Button add_auction;

  @FXML private TextField name_id;
  @FXML private ImageView backid;
  @FXML private ImageView logoId;

  @FXML private DatePicker startDate_id;
  @FXML private DatePicker endDate_id;

  @FXML private TextField threshold_id;
  @FXML private Text end_alert;

  @FXML private Text name_alert1;
  @FXML private Text threshold_alert;
  @FXML private Text start_alert;
  ...
  private boolean validateFields() throws SQLException {
    ...
    if (name_id.getText().isEmpty() || startDate_id.getValue() == null ||
        endDate_id.getValue() == null || threshold_id.getText().isEmpty()) {
  ...
  void addAuction(ActionEvent event) {
    ...
    try {
      if (validateFields()) {
        ...
        //    public Auction( String auction_name,Date start_date, Date
        //    end_date, float threshold,int aid) {
        AS.addd(new Auction(name, Date.valueOf(startDate_id.getValue()),
                            Date.valueOf(endDate_id.getValue()), threshold, 8,
                            currentUser.getUid()));
  ...
  public void initialize(URL url, ResourceBundle resourceBundle) {
    ...
    endDate_id.valueProperty().addListener((observable, oldValue, newValue) -> {
      if (startDate_id.getValue() != null &&
          newValue != null) { // Check if start date is not null
        LocalDate startDateValue = startDate_id.getValue();
        // Check if the end date is before or the same as the start date
        if (newValue.isBefore(startDateValue) ||
            newValue.isEqual(startDateValue)) {
