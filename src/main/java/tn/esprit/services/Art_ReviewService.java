...
public class Art_ReviewService implements IService<Art_Review> {
  ...
  public Art_ReviewService() { con = MyDatabase.getInstance().getConn(); }

  @Override
  public void add(Art_Review Art_Review) throws SQLException {
    String query =
        "INSERT INTO `review`( `Art_ref`, `uid`, `Date_published`, `Rating`, `Comment`) VALUES ('" +
        Art_Review.getArt_Ref() + "','" + Art_Review.getUid() + "','" +
        Art_Review.getDate_published() + "','" + Art_Review.getRating() +
        "','" + Art_Review.getComment() + "')";
  ...
  public void update(Art_Review Art_Review) throws SQLException {
    String query =
        "UPDATE review SET Art_ref=?, uid=?, Date_published=?, Rating=?, comment=? WHERE Review_id=?";
    PreparedStatement ps = con.prepareStatement(query);
  ...
  public List<Art_Review> diplayList() throws SQLException {
    ...
    while (res.next()) {
      ...
      Art_Review a = new Art_Review(Review_id, art_ref, uid, date_published,
                                    rating, comment);
      Art_Reviews.add(a);
    }
    return Art_Reviews;
