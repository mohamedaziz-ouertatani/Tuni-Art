...
public class UserService implements IService<User> {
  ...
  public UserService() { con = MyDatabase.getInstance().getConn(); }

  @Override
  public void add(User user) throws SQLException {
    String query =
        "INSERT INTO `user`( `fname`, `lname`, `email`, `gender`, `phone_nb`, `birth_date`, `password`) VALUES ('" +
        user.getFname() + "','" + user.getLname() + "','" + user.getEmail() +
        "','" + user.getGender() + "','" + user.getPhone_nb() + "','" +
  ...
  public void addd(User user) throws SQLException {
    if (user instanceof Admin) {
      ...
      String query =
          "INSERT INTO `user`(`fname`, `lname`, `email`, `gender`, `phone_nb`, `birth_date`,`profile_pic`, `password`, `role`) VALUES (?,?,?,?,?,?,?,?,?)";
      ...
      String query =
          "INSERT INTO `user`(`fname`, `lname`, `email`, `gender`, `status`, `phone_nb`, `birth_date`,`profile_pic`, `password`, `role`) VALUES (?,?,?,?,?,?,?,?,?,?)";
      PreparedStatement ps = con.prepareStatement(query);
  ...
  public void update(User user) throws SQLException {
    if (user instanceof Admin) {
      Admin admin = (Admin)user;
      String query =
          "UPDATE user SET fname=?, lname=?, email=?, gender=?, phone_nb=?, birth_date=?, password=?, profile_pic=?, role=? WHERE uid=?";
      PreparedStatement ps = con.prepareStatement(query);
      ...
      Artist artist = (Artist)user;
      String query =
          "UPDATE user SET fname=?, lname=?, email=?, gender=?, phone_nb=?, birth_date=?, password=?, verification_code=?, profile_pic=?, role=?, status=?, biography=?, portfolio=?, profileViews=? WHERE uid=?";
      PreparedStatement ps = con.prepareStatement(query);
      ...
      String query =
          "UPDATE user SET fname=?, lname=?, email=?, gender=?, phone_nb=?, birth_date=?, password=?, verification_code=?, profile_pic=?, role=?, status=?, profileViews=? WHERE uid=?";
      PreparedStatement ps = con.prepareStatement(query);
  ...
  public List<User> diplayList() throws SQLException {
    ...
    while (res.next()) {
      ...
      User ur = null;
      if (role.equals("User")) {
        ur = new User(uid, fname, lname, email, gender, status, phone_nb,
                      birth_date, profile_pic, password, verification_code,
                      role, profileViews);
      } else if (role.equals("Admin")) {
        ur = new Admin(uid, fname, lname, email, gender, phone_nb, birth_date,
                       password, role);
      } else if (role.equals("Artist")) {
        ur = new Artist(uid, fname, lname, email, gender, status, phone_nb,
                        birth_date, profile_pic, password, verification_code,
                        role, biography, portfolio, profileViews);
      }
      ...
      try (ResultSet res = statement.executeQuery()) {
        if (res.next()) {
          ...
          if (role.equals("User")) {
            ...
            ur = new Admin(uid, fname, lname, email, gender, phone_nb,
                           birth_date, password, role);
          } else if (role.equals("Artist")) {
            ur =
                new Artist(uid, fname, lname, email, gender, status, phone_nb,
                           birth_date, profile_pic, password, verification_code,
                           role, biography, portfolio, profileViews);
          }
        }
      ...
      try (ResultSet res = statement.executeQuery()) {
        if (res.next()) {
          ...
          if (role.equals("Artist")) {
            ur = new Artist(uid, fname, lname, email, gender, status, phone_nb,
                            birth_date, profile_pic, password,
                            verification_code, role, biography, portfolio);
          }
        }
      }
    }
  ...
  public void addFollower(int follower_id, int following_id)
      throws SQLException {
    String query =
  ...
  public void deleteFollower(int follower_id, int following_id)
      throws SQLException {
    String query =
  ...
  public int countFollowers(int user_id) throws SQLException {
    ...
    int count =
        rs.getInt(1); // Get the count from the first column of the result set
  ...
  public List<User> searchByName(String input) throws SQLException {
    String query =
        "SELECT * FROM user WHERE fname LIKE CONCAT('%', ?, '%') OR lname LIKE CONCAT('%', ?, '%')";
