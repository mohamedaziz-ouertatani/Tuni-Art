...
public class Art_PieceService implements IService<Art_Piece> {
  ...
  public Art_PieceService() { con = MyDatabase.getInstance().getConn(); }

  @Override
  public void add(Art_Piece Art_Piece) throws SQLException {
    String query =
        "INSERT INTO `art`( `art_title`, `art_price`, `artist_id`, `type`, `creation`, `description`, `style`,`image_id`,`music_path`) VALUES ('" +
        Art_Piece.getArt_title() + "','" + Art_Piece.getArt_price() + "','" +
        Art_Piece.getaid() + "','" + Art_Piece.getType() + "','" +
        Art_Piece.getCreation() + "','" + Art_Piece.getDescription() + "','" +
        Art_Piece.getStyle() + "','" + Art_Piece.getImage_path() + "','" +
        Art_Piece.getMusic_path() + "')";
  ...
  public void update(Art_Piece Art_Piece) throws SQLException {
    String query =
        "UPDATE art SET  art_title=?,art_price=?,type=?,creation=?,description=?,style=?,artist_id=?,image_id=?,music_path=? WHERE art_ref=?";
    PreparedStatement ps = con.prepareStatement(query);
  ...
  public List<Art_Piece> diplayList() throws SQLException {
    ...
    while (res.next()) {
      ...
      Art_Piece a =
          new Art_Piece(Art_ref, Art_title, Art_price, aid, Type, Creation,
                        Description, Style, image_id, music_path);
      Art_Pieces.add(a);
    }
    return Art_Pieces;
  }
  ...
  public List<Art_Piece> searchByTitle(String input) throws SQLException {
    ...
    ResultSet res =
        ps.executeQuery(); // Corrected: Remove the query parameter here
    List<Art_Piece> Art_Pieces = new ArrayList<>();
    while (res.next()) {
      int Art_ref = res.getInt(1);
      String Art_title = res.getString(2);
