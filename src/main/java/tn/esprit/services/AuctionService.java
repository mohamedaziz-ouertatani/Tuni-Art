...
public class AuctionService implements IService<Auction> {
  ...
  public AuctionService() {
    con = MyDatabase.getInstance()
              .getConn(); // it's the singleton(concept)to establish only one
  ...
  public void add(Auction auction) throws SQLException {
    String query =
        "INSERT INTO `auction`(`auction_ref`, `auction_name`, `start_date`, `end_date`, `threshold`, `art_ref`, `uid`) VALUES ('" +
        auction.getAuction_ref() + "','" + auction.getAuction_name() + "','" +
        auction.getStart_date() + "','" + auction.getEnd_date() + "','" +
        auction.getThreshold() + "','" + auction.getArt_ref() + "','" +
        auction.getUid() + "')";
  ...
  public void addd(Auction auction) throws SQLException {
    String query =
        "INSERT INTO `auction`(`auction_name`, `start_date`, `end_date`, `threshold`, `art_ref`,`uid`) VALUES (?,?,?,?,?,?)";
  ...
  public void update(Auction auction) throws SQLException {
    String query =
        "UPDATE auction SET  auction_name=?, start_date=?, end_date=?, threshold=?, art_ref=?, uid=? WHERE auction_ref=?";
    PreparedStatement ps = con.prepareStatement(query);
  ...
  public void updateThreshold(float newThreshold, int auctionId)
      throws SQLException {
    String updateQuery =
        "UPDATE auction SET threshold = ? ,interactions = interactions + 1 WHERE auction_ref = ?";
  ...
  public List<Auction> diplayList() throws SQLException {
    ...
    while (res.next()) {
      ...
      Auction a = new Auction(auction_ref, auction_name, start_date, end_date,
                              threshold, art_ref, uid);
      auctions.add(a);
    }
    return auctions;
  }
