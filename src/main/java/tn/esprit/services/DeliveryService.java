...
public class DeliveryService implements IService<Delivery> {
  ...
  public void update(Delivery delivery) throws SQLException {
    String query =
        "UPDATE delivery SET order_id = ?, estimated_date = ?, delivery_fees = ?, destination = ?, state = ?, agency_id = ? WHERE delivery_id = ?";
    try (PreparedStatement statement = connection.prepareStatement(query)) {
    ...
    try (Statement statement = connection.createStatement()) {
      ...
      while (resultSet.next()) {
        ...
        delivery.setEstimatedDate(
            resultSet.getDate("estimated_date").toLocalDate());
  ...
  public List<Delivery> search(String query) throws SQLException {
    List<Delivery> deliveries = new ArrayList<>();
    String sql =
        "SELECT * FROM delivery WHERE delivery_id LIKE ? OR order_id LIKE ? OR estimated_date LIKE ? OR delivery_fees LIKE ? OR destination LIKE ? OR state LIKE ? OR agency_id LIKE ?";
